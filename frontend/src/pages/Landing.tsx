  ...
  const capabilities = [
    {
      ...
      description: 'Automatische Bewertung jedes Anrufs von 1-10 für maximale Effizienz',
      features: ['Sentiment-Analyse', 'Keyword-Erkennung', 'Kaufbereitschaft-Score'],
      highlight: true
    },
    {
      title: '💰 30% mehr Umsatz',
      description: 'Systematische Follow-Ups steigern Ihre Conversion-Rate nachweislich',
      features: ['3/7/14/30 Tage Follow-Up', 'Personalisierte Scripts', 'Automatische Planung'],
      highlight: true
    },
    {
      title: '🔄 Lead Reaktivierung',
      description: 'Kalte Leads werden automatisch nach 30/60/90/180 Tagen reaktiviert',
      features: ['Spezielle Win-Back Angebote', 'Timing-Optimierung', 'Erfolgs-Tracking']
    },
    {
      title: '👥 Kunde hat die Wahl',
      description: 'Immer die Option zwischen KI-Assistent und echtem Menschen',
      features: ['Intelligentes Routing', 'Abteilungs-Transfer', 'Nahtlose Übergabe']
    },
    {
  ...
  return (
    <div className={`min-h-screen ${darkMode ? 'dark bg-slate-900' : 'bg-gray-50'}`}>
      ...
      <main className="pt-16">
        ...
        {/* KI-Maschine Philosophy Section */}
        <section className={`py-20 ${darkMode ? 'bg-gradient-to-br from-slate-900 to-slate-800' : 'bg-gradient-to-br from-gray-50 to-white'}`}>
          <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <div className="text-center mb-12">
              <div className="inline-block px-4 py-2 bg-gradient-to-r from-orange-500 to-red-500 text-white text-sm font-bold rounded-full mb-4">
                🚀 Die KI-Maschine Philosophie
              </div>
              <h2 className={`text-4xl font-bold ${darkMode ? 'text-white' : 'text-slate-900'} mb-4`}>
                Schluss mit "Shiny Object" KI-Spielereien
              </h2>
              <p className={`text-xl ${darkMode ? 'text-slate-300' : 'text-slate-600'} max-w-3xl mx-auto`}>
                Während andere noch mit 100% Automatisierung experimentieren, liefern wir profitable Systeme die <span className="font-bold text-teal-500">30% mehr Umsatz</span> generieren.
              </p>
            </div>

            <div className="grid md:grid-cols-3 gap-8 mb-12">
              <div className={`p-6 rounded-xl ${darkMode ? 'bg-slate-800/50' : 'bg-white'} shadow-lg`}>
                <div className="text-4xl mb-4">❌</div>
                <h3 className={`text-xl font-bold ${darkMode ? 'text-white' : 'text-slate-900'} mb-2`}>
                  Was wir NICHT sind
                </h3>
                <ul className={`space-y-2 ${darkMode ? 'text-slate-400' : 'text-slate-600'}`}>
                  <li>• 100% Automatisierung um jeden Preis</li>
                  <li>• Komplexe KI ohne ROI</li>
                  <li>• Technologie-Spielerei</li>
                  <li>• Kunden-Frustration</li>
                </ul>
              </div>

              <div className={`p-6 rounded-xl ${darkMode ? 'bg-gradient-to-br from-teal-900/30 to-blue-900/30' : 'bg-gradient-to-br from-teal-50 to-blue-50'} shadow-lg border-2 ${darkMode ? 'border-teal-500/50' : 'border-teal-500'}`}>
                <div className="text-4xl mb-4">✅</div>
                <h3 className={`text-xl font-bold ${darkMode ? 'text-white' : 'text-slate-900'} mb-2`}>
                  Was wir SIND
                </h3>
                <ul className={`space-y-2 ${darkMode ? 'text-slate-300' : 'text-slate-700'} font-medium`}>
                  <li>• Systematische Lead-Bewertung</li>
                  <li>• Messbare Umsatzsteigerung</li>
                  <li>• Kunde wählt: KI oder Mensch</li>
                  <li>• Profitables Wachstum</li>
                </ul>
              </div>
              ...
              <div className={`p-6 rounded-xl ${darkMode ? 'bg-slate-800/50' : 'bg-white'} shadow-lg`}>
                ...
                <ul className={`space-y-2 ${darkMode ? 'text-slate-400' : 'text-slate-600'}`}>
                  <li className="font-bold text-green-500">• +30% mehr Umsatz</li>
                  <li>• 5x höhere Conversion</li>
                  <li>• 80% weniger verlorene Leads</li>
                  <li>• 100% Kundenzufriedenheit</li>
                </ul>
              </div>
            </div>

            <div className={`p-8 rounded-2xl ${darkMode ? 'bg-gradient-to-r from-teal-900/50 to-blue-900/50' : 'bg-gradient-to-r from-teal-100 to-blue-100'} text-center`}>
              <h3 className={`text-2xl font-bold ${darkMode ? 'text-white' : 'text-slate-900'} mb-4`}>
                Das Geheimnis: Ihre Kunden haben IMMER die Wahl
              </h3>
              <p className={`text-lg ${darkMode ? 'text-slate-300' : 'text-slate-700'} mb-6 max-w-2xl mx-auto`}>
                "Ich kann Ihnen gerne dabei helfen oder Sie direkt mit einem Mitarbeiter verbinden. Was wäre Ihnen lieber?"
              </p>
              <div className="flex flex-wrap justify-center gap-4">
                <div className={`px-6 py-3 rounded-lg ${darkMode ? 'bg-slate-800' : 'bg-white'} shadow-md`}>
                  <span className="text-3xl font-bold text-teal-500">87%</span>
                  <p className={`text-sm ${darkMode ? 'text-slate-400' : 'text-slate-600'}`}>wählen KI bei Routine</p>
                </div>
                <div className={`px-6 py-3 rounded-lg ${darkMode ? 'bg-slate-800' : 'bg-white'} shadow-md`}>
                  <span className="text-3xl font-bold text-blue-500">92%</span>
                  <p className={`text-sm ${darkMode ? 'text-slate-400' : 'text-slate-600'}`}>höhere Zufriedenheit</p>
                </div>
                <div className={`px-6 py-3 rounded-lg ${darkMode ? 'bg-slate-800' : 'bg-white'} shadow-md`}>
                  <span className="text-3xl font-bold text-green-500">0%</span>
                  <p className={`text-sm ${darkMode ? 'text-slate-400' : 'text-slate-600'}`}>Frustration</p>
                </div>
              </div>
            </div>
          </div>
              ...
              {capabilities.map((capability, idx) => (
                <div key={idx} className={`p-8 rounded-xl ${
                  capability.highlight 
                    ? darkMode 
                      ? 'bg-gradient-to-br from-teal-900/50 to-blue-900/50 border-2 border-teal-500/50' 
                      : 'bg-gradient-to-br from-teal-50 to-blue-50 border-2 border-teal-500'
                    : darkMode 
                      ? 'bg-slate-900/50 border border-slate-700' 
                      : 'bg-gray-50 border border-gray-200'
                } ${capability.highlight ? 'transform hover:scale-105 transition-transform' : ''}`}>
                  {capability.highlight && (
                    <div className="inline-block px-3 py-1 bg-gradient-to-r from-teal-500 to-blue-500 text-white text-xs font-bold rounded-full mb-3">
                      NEU: KI-Maschine Feature
                    </div>
                  )}
                  ...
                  <ul className="space-y-2">
                    {capability.features.map((feature, fidx) => (
                      <li key={fidx} className={`flex items-center gap-2 ${darkMode ? 'text-slate-300' : 'text-slate-700'}`}>
                        <svg className={`w-5 h-5 ${capability.highlight ? 'text-teal-400' : 'text-teal-500'}`} fill="currentColor" viewBox="0 0 20 20">
        ...
        <section className={`py-20 ${darkMode ? 'bg-gradient-to-r from-slate-900 to-slate-800' : 'bg-gradient-to-r from-slate-800 to-slate-900'}`}>
          <div className="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 text-center">
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              ...
              <button
                className="px-8 py-4 rounded-lg font-semibold bg-teal-600 text-white hover:bg-teal-700 transition-colors"
              >
                Kontakt aufnehmen
              </button>
            </div>
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col md:flex-row justify-between items-center">
            ...
            <div className="flex items-center gap-6">
              <a href="#" className={`text-sm ${darkMode ? 'text-slate-400 hover:text-white' : 'text-slate-600 hover:text-slate-900'} transition-colors`}>
                Datenschutz
              </a>
              <a href="#" className={`text-sm ${darkMode ? 'text-slate-400 hover:text-white' : 'text-slate-600 hover:text-slate-900'} transition-colors`}>
                Impressum
